...
impl SessionActor {
    ...
    pub(super) async fn handle_bridge_tool_success(
        ...
        // Whether the active model accepts image inputs. When false, every
        // image-embedding path below degrades to a text-only note so no pixel
        // data is sent to a text-only model.
                ...
                result.output
        {
            ...
            if !model_supports_vision {
                // Text-only model: do not embed pixels. Surface the file's
                // existence and basic metadata so the agent can still reason
                // about it (e.g. hand it to an OCR/extraction tool).
