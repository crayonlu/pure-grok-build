...
/// Whether managed config fetching is enabled (env > config.toml > fork-mode
/// default). Open mode is opt-in; xAI compatibility mode preserves upstream's
/// default-on behavior.
...
pub fn is_fetch_enabled() -> bool {
    ...
    crate::config::load_effective_config()
        ...
        .unwrap_or_else(|| !crate::agent::service_policy::mode_from_disk().is_open())
}
