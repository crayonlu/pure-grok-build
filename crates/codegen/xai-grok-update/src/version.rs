...
mod tests {
    ...
    fn test_cli_base_urls_override_uses_single_self_hosted_url() {
        ...
        match prev {
            Some(v) => unsafe { std::env::set_var(ENV_CLI_BASE_URL, v) },
            None => unsafe { std::env::remove_var(ENV_CLI_BASE_URL) },
        }
    }
    ...
    fn test_gh_release_repo_override_points_at_fork() {
        ...
        match prev {
            Some(v) => unsafe { std::env::set_var(ENV_UPDATE_REPO, v) },
            None => unsafe { std::env::remove_var(ENV_UPDATE_REPO) },
        }
    }
}
