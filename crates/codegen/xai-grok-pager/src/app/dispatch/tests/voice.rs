...
/// Voice is not subscription-gated in the BYOK fork. A legacy tier stamp must
/// not turn the keybinding into an x.ai upsell path.
#[test]
fn voice_keybinding_ignores_subscription_tier() {
    if !xai_grok_voice::AUDIO_SUPPORTED {
        return;
    }
    let mut app = test_app_with_agent();
    ...
    assert!(
        app.voice_listening(),
        "legacy subscription tiers must not gate BYOK voice"
    );
}
