...
///
/// **Gated on the remote settings flag only.** When voice isn't available
/// (flag off, or a build without audio capture) this is a **silent no-op** — no
/// toast — so users who don't have the feature see nothing. Dictation routes
/// into a prompt box: the active agent's prompt, or the dashboard's dispatch
/// (new-agent) input. On the session-less welcome screen (first launch) a
/// session is created first — via the gated [`dispatch_new_session`], so auth
/// and folder-trust are respected — so voice works from a cold start in one press.
