...
mod factory_tests {
    ...
    #[test]
    fn runtime_config_preserves_provider_input_type() {
        let config = MemoryEmbeddingConfig {
            provider: "api".into(),
            protocol: "cohere_v2".into(),
            model: Some("embed-v4.0".into()),
            input_type: Some("search_document".into()),
            ..Default::default()
        };
        let runtime = runtime_config_from_config(&config, "https://embed.example/v2")
            .expect("configured embedding should resolve");
        assert_eq!(
            runtime.request.defaults.get("input_type"),
            Some(&serde_json::Value::String("search_document".into()))
        );
    }
